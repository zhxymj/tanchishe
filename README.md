...
- 撞墙或撞到自己后游戏结束
- 开始菜单：Start、Difficulty、Sound、Reset High Score、Quit
...
- 深色街机背景 + 轻微网格纹理，整体按产品界面层级统一设计
- 棋盘居中展示，带边框、阴影、柔和边缘光和圆角视觉效果
- 右侧信息面板采用仪表板式信息卡，显示状态、难度、分数、最高分、时间、等级、速度和功能按钮
- 蛇身使用连贯圆形节点、连接段和颜色层次
- 蛇头带眼睛，可看出朝向
- 食物带发光、脉冲动画、吃到后的粒子和光圈扩散
- 按钮区分主操作和次级操作，包含 hover、pressed、颜色层级和轻微位移动画
